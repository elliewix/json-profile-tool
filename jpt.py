...
def submit(strvar, unique_cb, numeric_cb, html_cb):
    popup_window = tk.Tk()
    tk.Label(popup_window, text=f"File analyzed: {strvar}").pack()

    resultsfolder = jpt.main_processing(meta['pathobj'], meta['stem'], unique = unique_cb, numeric = numeric_cb, html = html_cb)
...
tk.Label(main_window, text="Write out HTML of profile?", anchor = 'e', width = 20).grid(row=4, column=0)
...
submit_btn = tk.Button(main_window, text="Execute analysis",
                       command=lambda: submit(meta['pathobj'], unique_var.get(), numeric_var.get(), html_var.get()))
